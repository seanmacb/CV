...
\begin{rSubsection}{University of Zürich}{Apr 2023 - Present}{Ph.D Student}{Prof. Marcelle Soares-Santos}
    ...
    \begin{itemize}
        \item Led \ac{LSSTCam} commissioning to Rubin Observatory in Cerro-Pachón in Chile, from receipt on summit through early science operations. Main responsibilities included verification and testing of the focal plane.
        \item Characterized defects in the \ac{LSSTCam} focal plane, including the picture-frame response in different detector types and transient surface level effects. Developed custom image processing tool to monitor the stability of defects in the \ac{LSSTCam} focal plane.
        \item Led the \ac{ToO} observing program during the Rubin science verification period (May 2025 - December 2025), including \ac{ToO} responses to \ac{GW} candidates S250725j and S251112cm. Coordinated the \ac{ToO} observing program during early science operations (January 2026 - present).
    \end{itemize}
...
\begin{itemize}
\item Assembled and modified opto-mechanical mounts and ground support equipment for \ac{LLAMAS} instrument according to assembly drawings. Verified delivery and tolerance of custom-fabricated parts using coordinate measurement machine.
\item Tested the efficiency of diffraction gratings at nominal spectrograph orientation. Modified existing test equipment to measure blaze-angle transmission. 
\item Integrated the spectrograph camera lenses into their bezels for the entire 24 camera spectrograph.
...
\begin{itemize}
\item Developed a analyses pipeline to concatenate single-day solar observations. Utilized data pipeline to collect spectrophotometry from different solar events.
\item Analyzed ground-based data with data collected at the International Space Station observations from the same period. Studied correlations between historical S-index, magnetic activity, total solar irradiance, and spectral solar irradiance.
...
03/2026 & Characterization of the DECam Local Volume Exploration Survey for O4c Dark Siren Measurement, LIGO-Virgo-KAGRA Collaboration Meeting, Pisa, Italy\\
